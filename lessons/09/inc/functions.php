...
function newMember($username, $password, $email, $first, $last, $type)
{
	...
	$_SESSION['database']->insert(
		"characters", "memberid, firstname, lastname, type, health, speed, intelligence, strength, agility, magic, mapid, x, y, z", 
...
function errorMsg($text)
{
	return "<p align=\"center\">
			<span class=\"errorMsg\">Oops!</span> " . stripslashes($text) . ".
		</p>";
}
...
{
	return "<p align=\"center\">
			<span class=\"successMsg\">Success!</span> " . stripslashes($text) . ".
		</p>";
}
...
{
	return "<p align=\"center\">
			<span class=\"questionMsg\">Huh.</span> " . stripslashes($text) . "?
		</p>";
}
		...
		for ($j = $ymin; $j <= $ymax; $j++) {
		    ...
		    if ($value) {
					...
					if ($member->curcharacter->x == $i && $member->curcharacter->y == $j) {
						echo "<td width=" . $map->tile_width . " height=" . $map->tile_height . " style=\"background-image: url('" . $_SERVER['PHP_SELF'] . "/../images/tile.php?map=" . $member->curcharacter->mapid . "&x=" . $map->tile_x($i, $j, $member->curcharacter->z) . "&y=" . $map->tile_y($i, $j, $member->curcharacter->z) . "');\"><center><font color=\"#FFFFFF\"><b>C</b></font></center></td>";
					} else { //regular tile
						...
						if (hasMonster($i, $j, $member->curcharacter->z, $member->curcharacter->mapid)) {
							echo "<center><font color=\"#0000FF\">M</font></center>"; //show the monster
						}
...
function parkMove($id, $direction)
{
	...
	if (getValue($data['x'], $data['y'], $data['map'], $data['z']) == "L") {
		...
		} else if ($direction == "back" && $data['z'] != 5) { //if they hit the down direction, move them down a level (if not at lowest level)
