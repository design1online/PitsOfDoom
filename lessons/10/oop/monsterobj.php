...
class monster extends character
{
	...
	**************/
	function monster($id = null, $type = null, $mapid = null, $x = null, $y = null, $z = null)
	{
		global $_SESSION;
	...
	**************/
	function create()
	{
		if (!$this->id)
		{
			$this->id = $this->database->insert($this->table, "type, mapid, x, y, z, speed, intelligence,
			strength, agility, magic, health, fightlvl, magiclvl, monster, active, firstname, money,
			magicamount, weapon, armor, spell, shield", "'$this->type', '$this->mapid', 
	...
	/**************
	* Purpose: remove the monster from the database
	* Precondition: none
	* Postcondition: monster has been removed from the database
	*	but all of it's information still exists in this object
	**************/
	function death()
	{
		if ($this->id)
			$this->database->delete($this->table, $this->where);

	}

	/**************
	* Purpose: update the monster's health to what it is now
	* Precondition: none
	* Postcondition: monster's health has been updated
	**************/
	function setHealth($value)
	{
		...
		{
			$this->health = $value;
			$this->database->update($this->table, "health=$value", $this->where);
		}
...
{
	if ($number > 1)
		$number = 1; //sorry, disabled so people won't abuse!
