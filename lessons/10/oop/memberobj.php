...
class member
{
   ...
   **************/
   function member($id)
   {
	  ...
	  if ($_SESSION['debug'])
		echo "Creating new member object #$id.<br>";
	...
	//now load the data for the map the character is currently on
	$this->map = new mapobj($this->curcharacter->mapid, $this->curcharacter->z);
...
This is an automated message, please do not respond to it (oh and I hate spam so none of that either!)";
   ...
   **************/
   ...
   {
		if (!$message)
			return errorMsg("You must enter a message");
			
		$message = addslashes($message);
		
		return $this->database->insert("chatroom", "mid, message", "'$this->id', '$message'");
   }
